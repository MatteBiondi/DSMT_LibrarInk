...
handle_request(write_loan, #{user := User, isbn := Isbn}, From)->
  Result = performe_operation(from_reservation_to_loan,[User,Isbn]),
  gen_server:reply(From, Result);

handle_request(write_reservation, #{user := User, isbn := Isbn}, From)->
  Result = performe_operation(add_book_reservation,[User,Isbn]),
  gen_server:reply(From, Result);
...
handle_request(delete_copy, Args,From)->
  Result =
    case Args of
      #{isbn := Isbn, id := Id} ->
        performe_operation(delete_book_copy,[Isbn, Id]);
      #{isbn := Isbn} ->
        performe_operation(delete_all_book_copies,[Isbn]);
      _ ->
  ...
  Result =
    case Args of
      #{user := User, isbn := Isbn, id := Id} ->
        performe_operation(delete_lent_book,[User, Isbn, Id]);
      #{isbn := Isbn} ->
        performe_operation(delete_lent_by_book,[Isbn]);
      #{user := User} ->
        performe_operation(delete_lent_book_by_user,[User]);
      _ ->
        {error, invalid_arguments}
    end,
  gen_server:reply(From, Result);
...
handle_request(delete_reservation, Args,From)->
  Result =
    case Args of
      #{user := User, isbn := Isbn} ->
        performe_operation(delete_book_reservation,[User, Isbn]);
      #{user := User} ->
        performe_operation(delete_books_reservation_by_user,[User]);
      #{isbn := Isbn} ->
        performe_operation(delete_book_reservations_by_book,[Isbn]);
      _ ->
        {error, invalid_arguments}
    end,
  gen_server:reply(From, Result);
...
handle_request(archive_loans, #{},From)->
  Result = performe_operation(get_and_delete_ended_loans,[]),
  gen_server:reply(From, Result);

handle_request(archive_reservations, #{},From)->
  Result = performe_operation(get_and_delete_ended_reservations,[]),
  gen_server:reply(From, Result);
...
handle_request(read_copies, Args,From)->
  Result =
    case Args of
      #{type := all, isbn := Isbn} ->
        performe_operation(copies_by_book,[Isbn]);
      #{type := all} ->
        performe_operation(all_copies_all_book,[]);
      #{type := available, operation := count, isbn := Isbn} ->
        performe_operation(count_available_copies_by_book,[Isbn]);
      #{type := available, operation := list, isbn := Isbn} ->
        performe_operation(list_available_copies_by_book,[Isbn]);
      _ ->
        {error, invalid_arguments}
    end,
  gen_server:reply(From, Result);
...
handle_request(read_loans, Args,From)->
  Result =
    case Args of
      #{isbn := Isbn} ->
        performe_operation(lent_copies_by_book,[Isbn]);
      #{user := User} ->
        performe_operation(loans_by_user,[User]);
      #{isbn := Isbn, id := Id} ->
        performe_operation(loan_by_book_copy,[Isbn, Id]);
      #{} ->
        performe_operation(all_pending_loans,[]);
      _ ->
        {error, invalid_arguments}
    end,
  gen_server:reply(From, Result);
...
handle_request(read_ended_loans, #{},From)->
  Result = performe_operation(all_ended_loans,[]),
  gen_server:reply(From, Result);
...
handle_request(read_reservations, Args,From)->
  Result =
    case Args of
      #{user := User, isbn := Isbn} ->
        performe_operation(reservations_by_user_and_book,[User, Isbn]);
      #{user := User} ->
        performe_operation(reservations_by_user,[User]);
      #{isbn := Isbn} ->
        performe_operation(reservations_by_book,[Isbn]);
      #{} ->
        performe_operation(all_pending_reservations,[]);
      _ ->
        {error, invalid_arguments}
    end,
  gen_server:reply(From, Result);
...
handle_request(read_ended_reservations, #{},From)->
  Result = performe_operation(all_ended_reservations,[]),
  gen_server:reply(From, Result);
...
-spec(performe_operation(Fun :: atom(), Args::map()) ->
  ...
  {error, error_not_available_copy | undefined_book_copy | error_no_loan_found | error_no_reservation_found |
    ...
    error_pending_loan_or_reservation | error_pending_reservation | undefined_book | E::term()}).
performe_operation(Fun, Args)->
  try
